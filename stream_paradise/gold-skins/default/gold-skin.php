  ...
  public function GOLD_head() {
	if($this->GOLD_REQUEST('gold') != 'post') {
		...
		elseif($this->GOLD_REQUEST('gold') == 'login') {
			$title = $LANG['login']." &raquo; ".$this->set('gold_website_title');
		}
		elseif($this->GOLD_REQUEST('gold') == 'register') {
			$title = $LANG['register']." &raquo; ".$this->set('gold_website_title');
		}
		elseif($this->GOLD_REQUEST('gold') == 'forgot') {
			$title = $LANG['forgot_your_password']." &raquo; ".$this->set('gold_website_title');
		}
		elseif($this->GOLD_REQUEST('gold') == 'submit') {
			$title = $LANG['add_image']." &raquo; ".$this->set('gold_website_title');
		}
		...
		elseif($this->GOLD_REQUEST('gold') == 'pages' && $this->GOLD_REQUEST('sub_gold') == '') {
			$title = $LANG['pages']." &raquo; ".$this->set('gold_website_title');
		}
		elseif($this->GOLD_REQUEST('gold') == 'pages' && $this->GOLD_REQUEST('sub_gold') != '') {
			$page = mysql_fetch_array(mysql_query("SELECT * FROM gold_pages WHERE name='".$this->GOLD_REQUEST('sub_gold')."'"));
			$title = $page['title']." &raquo; ".$LANG['pages']." &raquo; ".$this->set('gold_website_title');
		}
		elseif($this->GOLD_REQUEST('gold') == 'sort') {
			if($this->GOLD_REQUEST('sub_gold') == 'title') {
				$title = $LANG['title']." &raquo; ".$LANG['filter']." &raquo; ".$this->set('gold_website_title');
			}
			elseif($this->GOLD_REQUEST('sub_gold') == 'rating') {
				$title = $LANG['rating']." &raquo; ".$LANG['filter']." &raquo; ".$this->set('gold_website_title');
			}
			elseif($this->GOLD_REQUEST('sub_gold') == 'year') {
				$title = $LANG['year_title']." &raquo; ".$LANG['filter']." &raquo; ".$this->set('gold_website_title');
			}
			elseif($this->GOLD_REQUEST('sub_gold') == 'views') {
				$title = $LANG['views']." &raquo; ".$LANG['filter']." &raquo; ".$this->set('gold_website_title');
			}
			elseif($this->GOLD_REQUEST('sub_gold') == 'imdb') {
				$title = "IMDB"." &raquo; ".$LANG['filter']." &raquo; ".$this->set('gold_website_title');
			}
		}
	...
	<script src="'.$this->getCurrentTemplatePath().'/jquery-1.8.0.min.js" type="text/javascript"></script>
</head>
		');
	} else {
		$query = mysql_query("SELECT * FROM gold_posts WHERE post_name='".$this->GOLD_REQUEST('sub2_gold')."' order by post_id DESC LIMIT 1");
        ...
        <link rel="stylesheet" href="'.$this->getCurrentTemplatePath().'/font-awesome.min.css">
       	<script src="'.$this->getCurrentTemplatePath().'/jquery-1.8.0.min.js" type="text/javascript"></script>
		<script type="text/javascript" src="'.$this->getCurrentTemplatePath().'/player/goldplayer.js"></script>
</head>
		');
		}
	}
...
if ($this->GOLD_logged_in()) {
	$GOLD_html .= $this->output('<a href="'.$this->GOLD_ROOT().'admin" style="text-decoration: none; background: #353535; color: #fff; display: inline-block; position: absolute; margin-left: -90px; margin-top: 15px; padding: 4px 15px;">'.$this->LANG('admin').'</a>');
	$GOLD_html .= $this->output('<a href="'.$this->GOLD_ROOT().'logout" style="text-decoration: none; background: #353535; color: #fff; display: inline-block; position: absolute; margin-left: -185px; margin-top: 15px; padding: 4px 15px;">'.$this->LANG('logout').'</a>');
}
...
<div id="sub-bar-container" class="sub-bar-sort" style="top: 0px;">
	<div id="sub-bar" class="headline-bar wrap cf">
		<div style="padding: 0px 30px; margin-bottom: 35px;">
					<div class="select-movie-genre" id="select-genre" style="left:30px; display:block">
                        <div class="select-genre-title">
                            <a>'.$this->LANG('choose_movies_genre').'</a>
                        </div>
                        <div id="select-movie-genre" class="select-genre-categories" style="display: none;">
                            <div class="select-genre-categories-inner">
                                <ul>');
				$genre[query] = mysql_query("SELECT * FROM gold_categories WHERE status='1' AND type = 'movie' ");
        ...
        $GOLD_body .= $this->output('
                    ...
                    <div id="sort-by">
                        <div id="filter-by">
                        	<span>'.$this->LANG('filter_by').':</span>
                        	<a href="'.$this->GOLD_ROOT().'sort/release_date/'.$cat_sort.'" class="views-sort-btn" data-order="desc" title="'.$this->LANG('release_date').'">'.$this->LANG('release_date').'</a> <span class="filter-line"></span>
                        	<a href="'.$this->GOLD_ROOT().'sort/year/'.$cat_sort.'" class="date-sort-btn" data-order="desc" title="'.$this->LANG('movie_year').'">'.$this->LANG('movie_year').'</a> <span class="filter-line"></span>
                        	<a href="'.$this->GOLD_ROOT().'sort/views/'.$cat_sort.'" class="views-sort-btn" data-order="desc" title="'.$this->LANG('views').'">'.$this->LANG('views').'</a> 
                        </div>
                    </div>
        </div>
	</div>
  ...
  public function GOLD_aside_sidebar() {
		...
		$GOLD_html .= $this->output('<aside class="sidebar">
			<div class="aside-header">
				<div class="aside">
					<h2 style="margin-left: 20px;">'.$this->LANG('menu').'</h2>
					<a class="close-aside" id="close-aside" style="margin-right: 20px;">'.$this->LANG('sidebar_close').'</a>
				</div>
			</div>
			');
		$GOLD_html .= $this->output('<ul class="clearfix">');
		$GOLD_html .= $this->output('<li><a href="'.$this->GOLD_ROOT().'genre" class="icon-newest"><span class="filter genre"></span>'.$this->LANG('sidebar_genres').'</a></li>');
		$GOLD_html .= $this->output('<li><a href="'.$this->GOLD_ROOT().'pages" class="icon-pages"><span class="filter pages"></span>'.$this->LANG('sidebar_pages').'</a></li>');
		$GOLD_html .= $this->output('<li><a href="'.$this->GOLD_ROOT().'sort/hot/" class="icon-topuser"><span class="filter popular_movies"></span>'.$this->LANG('sidebar_popular_movies').'</a></li>');
		$GOLD_html .= $this->output('<li><a href="'.$this->GOLD_ROOT().'pages/feedback" class="icon-feedback"><span class="filter feedback"></span>'.$this->LANG('sidebar_feedback').'</a></li>');
	  	...
	  	if($check_episodes >= '1') {
	  		while($episode = mysql_fetch_array($select_episodes)) {
	  			if($episode['movie_link'] != '') {
	  				$post_type .= '
	  				<div id="EPISODE_PLAYER" class="Gold_PLAYER" style="max-height: 470px;"></div>
		            <script type="text/javascript">
		            $(".Gold_PLAYER").goldplayer({
					    src: "'.$episode['movie_link'].'",
					    poster: "/gold-app/gold-uploads/media/'.$media['post_thumb'].'"
					});
		            </script>
					';
	  			}
	  			...
	  			$select_all_episodes = mysql_query("SELECT (@row:=@row+1) AS ROW_ID, e.* FROM gold_episodes e, (SELECT @row := 0) r WHERE movie_id='".$media['post_name']."' GROUP BY season_id ORDER BY season_id ASC");
	  			...
	  			$media_post_name = "'".$media['post_name']."'";

	  			$post_type .= '
	  			<div id="episodes">
	  			<style>
	  			div.episode-item { min-width: 216px; }
	  			a#SeasonsOpen {
				        position: absolute;
					    background: #282828;
	  				...
	  				<a href="javascript:" onclick="show_seasons('.$media_post_name.');" id="SeasonsOpen" title="Seasons" style="display: none;">Seasons</a>
...
';
	...
	foreach ($trailer['youtube'] as $key_3 => $trailers) {
  	...
  	//LINKS
    $GOLD_html .= $this->output('<div class="post_preview">');
    $GOLD_html .= $this->output('<div id="left-wrapper"></div>');
    $GOLD_html .= $this->output('<div id="right-wrapper"></div>');
    $links_req = mysql_query("SELECT * FROM gold_links WHERE post_id='".$media['post_id']."'");
    while($link = mysql_fetch_array($links_req)) {
          $post_type.='<a href="'.$link['link'].'" ">LINK</a>';
      }
      $GOLD_html .= $this->output('<div class="player" >'.$post_type.'</div></div>');
      $GOLD_html .= $this->output('<div class="wrap cf" style="max-width: 920px;"><div class="full_wrap cf">');
      /*while($link = mysql_fetch_array($links_req)) {
        $post_type='<iframe src="'.$link['link'].'" frameborder="0" style="width: 920px; height: 510px;"></iframe>';
        $GOLD_html .= $this->output('
		<div class="player">
		'.$post_type.'
...
if(mysql_num_rows($fb_comments_sql) == '1') { $fbcom_active = "active"; } else { $fbcom_hidden = "style='display: block;'"; } $disqus_active = "";
$GOLD_html .= $this->output('
<div style="background: #fff; border: 1px solid #E7E7E7; border-radius: 3px;">
    <ul class="comments_switch">
		...
		<li><a class="comments-switch" data-comments="#tab_disqus_comments" href="javascript:void(0);">
			<img src="https://a.disquscdn.com/dotcom/d-c213289/img/brand/disqus-social-icon-blue-white.svg" width="34" style="fill: #fff; position: relative; top: 3px; margin: 0px 10px 0px 0px;">
			<span style="position: relative; top: -9px;">'.$this->LANG('comments').'</span>
		</a></li>
    </ul>
	<div class="comments_tab">
		...
		<section id="tab_disqus_comments" class="comments_section" style="display: none;">
			<div id="disqus_thread" style="color: #000;"></div>
			<script type="text/javascript">
			    /* * * CONFIGURATION VARIABLES * * */
			    var disqus_shortname = "'.$this->widget_echo('disqus').'";
...
$blocks_sql = mysql_query("SELECT * FROM gold_blocks WHERE block_type='post' AND block_status='1' ORDER BY block_position ASC");
  ...
  public function GOLD_sort_page_category($sort, $category) {
	...
	if($sort == '') {
	    ...
	    $q = mysql_query("SELECT * FROM gold_posts WHERE category_id LIKE '%".$category."%' AND post_status='1' ORDER BY year DESC LIMIT ".$limit."");
	}
      elseif($sort == 'release_date') {
        $q = mysql_query("SELECT * FROM gold_posts WHERE category_id LIKE '%".$category."%' AND post_status='1' ORDER BY rls_date DESC LIMIT ".$limit."");
    }
	elseif($sort == 'imdb') {
  ...
  public function GOLD_sort_page($sort) {
	...
	if($sort == '') {
	    ...
	    $q = mysql_query("SELECT * FROM gold_posts WHERE post_status='1' ORDER BY year DESC LIMIT ".$limit."");
	} elseif($sort == 'imdb') {
    } elseif($sort == 'release_date') {
        $q = mysql_query("SELECT * FROM gold_posts WHERE post_status='1' ORDER BY rls_date DESC LIMIT ".$limit."");
    } elseif($sort == 'imdb') {
	...
	$GOLD_html .= $this->output('</div>');
    //$GOLD_html .= $this->pagination('gold_posts', 'rls_date', 10);
    return $GOLD_html;
  }
  ...
  public function GOLD_categories_page() {
	...
	while($media = mysql_fetch_array($q)) {
		$media_post_num = mysql_num_rows(mysql_query("SELECT * FROM gold_posts WHERE category_id LIKE '".$media['category_id']."' AND post_status='1' order by post_id DESC"));
		...
		while($media2 = mysql_fetch_array($q2)) {
			$media2_post_num = mysql_num_rows(mysql_query("SELECT * FROM gold_posts WHERE category_id LIKE '".$media2['category_id'].",' AND post_status='1' order by post_id DESC"));
			...
			<div class="side_box" style="margin-bottom: 20px;width: 144px;float: left;margin-right: 10px;">
				...
				<span class="content" style="margin-top: 5px; margin-bottom: 0px;width: 144px;">
					<div style="margin-bottom: -1px;">
						<ul class="categories">';
							$query_of_categories = mysql_query("SELECT * FROM gold_categories WHERE status='1' and type = 'movie' ");
							while($category = mysql_fetch_array($query_of_categories)) {
								if($this->GOLD_REQUEST("sub_gold") == $category['name']) { $active = "style='color: rgb(107, 79, 100); font-weight: bold; text-decoration: none;'"; } elseif($this->GOLD_REQUEST("sub_gold") != $category['name']) { $active = ''; }
...
<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0],p=/^http:/.test(d.location)?"http":"https";if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src=p+"://platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script></div>';
		...
		while($media = mysql_fetch_array($q)) {
			...
			if($this->GOLD_REQUEST('sub2_gold') == '') {
				...
				$GOLD_html .= $this->output('<div class="wrap_normal container_page" style="width: 592px; float: left;">');
					$no_sql = mysql_query("SELECT posts.*, votes.* FROM gold_posts posts, gold_favorites votes WHERE posts.post_id=votes.post_id AND votes.user_id='".$media['user_id']."' AND posts.post_status='1' ORDER BY posts.post_id ASC");
					if(mysql_num_rows($no_sql) == '0') {
						$GOLD_html .= $this->output('<div class="no_results">');
							$GOLD_html .= $this->output('<img src="'.$this->getCurrentTemplatePath().'/images/icon-results.png" style="width: 111px;">');
							$GOLD_html .= $this->output('<span style="font-size: 25px;font-weight: normal;display: block;">'.$this->LANG('no_results').'</span>');
						...
						$GOLD_html .= $this->GOLD_box(mysql_query("SELECT posts.*, votes.* FROM gold_posts posts, gold_favorites votes WHERE posts.post_id=votes.post_id AND votes.user_id='".$media['user_id']."' AND posts.post_status='1' ORDER BY posts.post_id ASC LIMIT ".$limit.""), $content, "favorites");
					}
				$GOLD_html .= $this->output('</div>');
			}
  ...
  public function GOLD_submit() {
	...
	if($this->GOLD_REQUEST('sub_gold') == '' || $this->GOLD_REQUEST('sub_gold') == 'image') {
	  ...
	  if($_REQUEST['error'] == '1') {
			$error['form'] = '<font style="color: #000; color: rgb(253, 3, 3); padding-bottom: 10px; display: inline-block; font-weight: bold;">'.$this->LANG('please_fill_out_important_fields').' !</font><br />';
	  }
...
<a href="/admin" id="back_to" class="back_to btn" style="margin-top: -16px; position: absolute; right: 178px;">
	<font style="margin-top: 1px; display: inline-block;">'.$this->LANG('back_to_admin').'</font>
</a>
	...
	<form class="wrap_normal" id="upload-form" action="'.$this->GOLD_ROOT().'gold-app/gold-includes/GOLD.php" method="POST" enctype="multipart/form-data">
		'.$error['form'].'
		<label>'.$this->LANG('movie_title').'</label><br />
		<input type="text" id="upload-title" name="title" class="field" pattern=".{3,128}" value="'.$_POST['title'].'" placeholder="'.$this->LANG('movie_title').'" required="required" style="margin-right: 8px;">
		
		<label>'.$this->LANG('movie_year').' & '.$this->LANG('imdb_rating').'</label><br />
		<input type="text" id="upload-title" name="year" class="field" pattern=".{3,128}" value="'.$_POST['year'].'" placeholder="'.$this->LANG('movie_year').'" required="required" style="width: 265px; float: left; margin-right: 8px;">
		<input type="text" id="upload-title" name="imdb" class="field" pattern=".{3,128}" value="'.$_POST['imdb'].'" placeholder="'.$this->LANG('imdb_rating').'" required="required" style="width: 265px; float: left;">
		
		<a id="FETCH_MOVIE" onclick="FETCH_MOVIE()">'.$this->LANG('fetch_movie').'</a>

		<label>'.$this->LANG('choose_movies_genre').'</label>
	...
	$GOLD_html .= $this->output('
		...
		<input type="text" id="upload-title" name="directed_by" class="field" pattern=".{3,128}" value="'.$_POST['directed_by'].'" placeholder="'.$this->LANG('producer_title').'" required="required">
		
		<input type="text" id="upload-title" name="casts" class="field" pattern=".{3,128}" value="'.$_POST['casts'].'" placeholder="'.$this->LANG('actors_title').'" required="required">
		
		<textarea id="upload-desc" name="description" class="field" placeholder="'.$this->LANG('movie_description').'" style="width: 565px;outline: 0px;height: 50px;" required="required">'.$_POST['description'].'</textarea>
		
		<span id="add-seperator" class="or-seperator" style="margin: 24px auto 34px;"><em>'.$this->LANG('upload_movie_cover').'</em></span>
		...
		<input id="SeasonButton" type="button" value="Create Season" onclick="add_season(1)" style="float: right!important; margin-top: 10px; margin-bottom: 10px; background-color: #EC3A39 !important; text-decoration: none; border: none; font-size: 16px; padding: 6px 18px; border-radius: 40px; color: #FFF; text-align: center; letter-spacing: 0.5px; position: relative; cursor: pointer; display: inline-block; overflow: hidden; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; vertical-align: middle; z-index: 1;">
		<div id="SeasonDIV-1">
			<input type="hidden" name="season[]" value="">
			<span id="add-seperator" class="or-seperator" style="margin: 24px auto 34px; border-color: #EC3A39;"><em style="font-size: 15px; font-family: Nunito; color: #EC3A39; font-weight: bold;">Season <span id="EPISODE_NUM">1</span></em></span>
			
			<span id="add-seperator" class="or-seperator" style="margin: 24px auto 34px;"><em>Episode <span id="EPISODE_NUM">1</span> (Season 1)</em></span>
			<div style="margin-top: -24px;">
				<div class="add-url-input cf"><input id="add-url" type="url" required="required" name="episode_name_1[]" class="field" placeholder="Episode Name"></div>
				<div class="add-url-input cf"><input id="add-url" type="url" required="required" name="episodes_movie_flv_1[]" class="field" placeholder="Episode Link (flv, mp4 and etc.)"></div>
				<div class="add-url-input cf"><input id="add-url" type="url" required="required" name="episodes_movie_iframe_1[]" class="field" placeholder="Episode Iframe Link"></div>
			</div>
			<input id="AddEpisode_1" type="button" value="Add Episode" onclick="add_episode(1,1)" style="float: right!important; margin-top: 10px; margin-bottom: 10px; background-color: #4CAF50 !important; text-decoration: none; border: none; font-size: 16px; padding: 6px 18px; border-radius: 40px; color: #FFF; text-align: center; letter-spacing: 0.5px; position: relative; cursor: pointer; display: inline-block; overflow: hidden; -webkit-user-select: none; -moz-user-select: none; -ms-user-select: none; vertical-align: middle; z-index: 1;">
		</div>
...
');
  ...
  public function GOLD_box($q, $content, $GET, $statement) {
	...
	if($this->set('gold_boxtype') == '1' || $GET == 'wall' && $GET == 'favorites') {
	  ...
	  if($media['post_title'] != '') {
		  	...
		  	if ($count % 35 == 5) {
		        ...
		        <div class="movie-element movie-element-size-1" data-id="9452">
		        	<div class="adverts" style="height: 291px; width: 217px; overflow: hidden; border-radius: 3px; background: #EC3A39;">
		        		<span style="padding-bottom: 5px; padding-top: 5px; color: #fff; font-size: 13px; font-family: Arial; display: inline-block;">'.$this->LANG('advert').'</span>
		...
		$GOLD_html .= $this->output('
	        <div class="movie-element movie-element-size-1" data-id="9452">
	        	<a href="'.$this->GOLD_ROOT().$GOLD_CATEGORY['name'].'/'.$media['post_name']. '" class="movie-image" style="background-color:#7d506f;background-image: url(' .$post_type.');">
  ...
  public function GOLD_admin_posts($table, $where, $per_page) {
	...
	elseif($this->GOLD_REQUEST('sub2_gold') == 'edit') {
		...
		if($_POST['submit']) {
			if(!$_POST['title']) { echo '<span class="fill_out_admin">'.$this->LANG('please_fill_out_important_fields').'</span>'; } else {
				...
				while($row_episode = mysql_fetch_array($query_episodes)) {
					$update = mysql_query("UPDATE `gold_episodes` SET `movie_link`='".$_POST['episodes_movie_flv_'.$row_episode['id']]."', `movie_iframe`='".$_POST['episodes_movie_iframe_'.$row_episode['id']]."', `episode_name`='".$_POST['episode_name_'.$row_episode['id']]."' WHERE id='".$row_episode['id']."'");
				}
		...
		<label>'.$this->LANG('movie_year').' & '.$this->LANG('imdb_rating').'</label><br />
		<div style="width: 102%;">
			<input type="text" id="upload-title" name="year" class="field" pattern=".{3,128}" value="'.$row['year'].'" placeholder="'.$this->LANG('movie_year').'" required="required" style="width: 48%; float: left; margin-right: 8px;">
			<input type="text" id="upload-title" name="imdb" class="field" pattern=".{3,128}" value="'.$row['imdb'].'" placeholder="'.$this->LANG('imdb_rating').'" required="required" style="width: 47%; float: left;">
		</div>
		<label>'.$this->LANG('producer_title').'</label><br />
		<input type="text" id="upload-title" name="directed_by" class="field" pattern=".{3,128}" value="'.$row['directed_by'].'" placeholder="'.$this->LANG('producer_title').'" required="required" style="width: 100%;">
		<label>'.$this->LANG('actors_title').'</label><br />
		<input type="text" id="upload-title" name="casts" class="field" pattern=".{3,128}" value="'.$row['casts'].'" placeholder="'.$this->LANG('actors_title').'" required="required" style="width: 100%;">
		<label>'.$this->LANG('movie_description').'</label><br />
		<textarea id="upload-desc" name="description" class="field" placeholder="'.$this->LANG('movie_description').'" style="width: 102%;outline: 0px;height: 50px;  display: inline-block;
  height: 100px;
  line-height: 24px;
  padding: 0px 7px;
  margin-bottom: 4px;
...
$episodes_re = mysql_query("SELECT (@row:=@row+1) AS ROW_ID, gold_episodes.* FROM gold_episodes gold_episodes, (SELECT @row := 0) r WHERE movie_id='".$row['post_name']."' GROUP BY season_id");
			while($episode = mysql_fetch_array($episodes_re)) {
				$GOLD_html .= $this->output('
					<style>.AddEpisode { display: none; }</style>
		<div id="SeasonDIV-'.$episode['season_id'].'">
			<input type="hidden" name="season[]" value="">
			<span id="add-seperator" class="or-seperator" style="margin: 24px auto 34px; border-color: #EC3A39;"><em style="font-size: 15px; font-family: Nunito; color: #EC3A39; font-weight: bold;">Season <span id="EPISODE_NUM">'.$episode['season_id'].'</span></em></span>
			
			');
				$episodes_re2 = mysql_query("SELECT (@row:=@row+1) AS ROW_ID, gold_episodes.* FROM gold_episodes gold_episodes, (SELECT @row := 0) r WHERE movie_id='".$row['post_name']."' AND season_id='".$episode['season_id']."' ORDER BY id DESC");
			while($episode2 = mysql_fetch_array($episodes_re2)) {
				if($episode2['season_id'] != '1') {
					$epd2 = '';
				}
				$gd = "'1'";
	$GOLD_html .= $this->output('
			<span id="add-seperator" class="or-seperator" style="margin: 24px auto 34px;"><em>Episode <span id="EPISODE_NUM">'.$episode2['ROW_ID'].'</span> (Season '.$episode2['season_id'].')</em></span>
			<div style="margin-top: -24px;">
				<div class="add-url-input cf"><input id="add-url" type="text" name="episode_name_'.$episode2['id'].'" class="field" value="'.$episode2['episode_name'].'" placeholder="Episode Name"></div>
				<div class="add-url-input cf"><input id="add-url" type="text" name="episodes_movie_flv_'.$episode2['id'].'" class="field" value="'.$episode2['movie_link'].'" placeholder="Episode Link (flv, mp4 and etc.)"></div>
				<div class="add-url-input cf"><input id="add-url" type="text" name="episodes_movie_iframe_'.$episode2['id'].'" class="field" value="'.$episode2['movie_iframe'].'" placeholder="Episode Iframe Link"></div>
			</div>
				');
}
$GOLD_html .= $this->output('</div>');
	...
	$GOLD_html .= $this->output('
        ...
        <table class="table table-bordered table-striped table-responsive">
            <thead>
                <tr>
                    <th><div class="th">
                        '.$this->LANG('admin_image').'
                    </div></th>
					<th><div class="th">
                        '.$this->LANG('admin_title').'
                    </div></th>
					<th><div class="th">
...
border-radius: 0;
  ...
  public function GOLD_admin_layout($table, $where) {
	if($this->GOLD_REQUEST('sub2_gold') == '') {
		...
		if($_POST['submit']) {
			if(!$_POST['gold_skin'] || !$_POST['gold_lang'] || !$_POST['gold_logo']) { echo '<span class="fill_out_admin">'.$this->LANG('please_fill_out_important_fields').'</span>'; } else {
				...
				$disqus_shortname = array(
					'widget_title' => "disqus",
					'widget_code' => mysql_real_escape_string($_POST['disqus_shortname'])
				);
				$gold_lang = array(
					'set_name' => "gold_lang",
				...
				$this->GOLD_DB_UPDATE("gold_widgets", $disqus_shortname, "WHERE widget_title = 'disqus'");
		...
		$GOLD_html .= $this->output('
				...
				<div class="field">
					<label>DISQUS Shortname (For Disqus Comments)</label>
					<input type="text" name="disqus_shortname" value="'.$this->widget_echo("disqus").'" style="width: 100%;border: 1px solid #EC3A39;border-radius: 3px;margin-top: 0px;">
				</div>
				<div class="field">
					<label>Email</label>
  ...
  public function footer() {
		...
		<footer id="footer" style="margin-bottom: -25px;">	
			...
			<span class="powered_by">2017 © Powered by <a target="_blank" style="color: rgb(234, 255, 0);">Stream Paradise</a>
