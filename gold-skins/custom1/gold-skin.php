...
echo "
  ...
  public function GOLD_head() {
	...
	<script src="'.$this->getCurrentTemplatePath().'/jquery-1.8.0.min.js" type="text/javascript"></script>
		...
		$query = mysql_query("SELECT * FROM gold_posts WHERE post_name='".$this->GOLD_REQUEST('sub2_gold')."' ORDER BY post_id ASC LIMIT 1");
        ...
        $GOLD_body .= $this->output('
                    ...
                    <div id="sort-by">
                        <div id="filter-by">
                        	...
                        	<a href="'.$this->GOLD_ROOT().'sort/title/'.$cat_sort.'" class="title-sort-btn" data-order="asc" title="'.$this->LANG('title').'">'.$this->LANG('title').'</a> <span class="filter-line"></span>
                        	<a href="'.$this->GOLD_ROOT().'sort/rating/'.$cat_sort.'" class="rating-sort-btn" data-order="desc" title="'.$this->LANG('rating').'">'.$this->LANG('rating').'</a> <span class="filter-line"></span>
                        	<a href="'.$this->GOLD_ROOT().'sort/year/'.$cat_sort.'" class="date-sort-btn" data-order="desc" title="'.$this->LANG('year').'">'.$this->LANG('year').'</a> <span class="filter-line"></span>
                        	<a href="'.$this->GOLD_ROOT().'sort/views/'.$cat_sort.'" class="views-sort-btn" data-order="desc" title="'.$this->LANG('views').'">'.$this->LANG('views').'</a> <span class="filter-line"></span>
...
$blocks_sql = mysql_query("SELECT * FROM gold_blocks WHERE block_type='post' AND block_status='1' ORDER BY block_position ASC");
  ...
  public function GOLD_categories_page() {
	...
	while($media = mysql_fetch_array($q)) {
		$media_post_num = mysql_num_rows(mysql_query("SELECT * FROM gold_posts WHERE category_id LIKE '".$media['category_id']."' AND post_status='1' ORDER BY post_id ASC"));
		...
		while($media2 = mysql_fetch_array($q2)) {
			$media2_post_num = mysql_num_rows(mysql_query("SELECT * FROM gold_posts WHERE category_id LIKE '".$media2['category_id'].",' AND post_status='1' ORDER BY post_id ASC"));
